 ...
 * 物理计算核心类
 * 
 * 这个类包含所有Minecraft运动物理的计算逻辑，是求解器的核心。
 * 
 * 主要功能：
 * 1. calculateJumpBM：基础跳跃计算，计算从初始向后速度开始，经过连跳后能达到的向前bm
 * 2. calculateDelayedJumpJumps：精确跳跃计算，支持指定起跳速度，用于优化
 * 3. calculateFinalJump：最终跳跃计算，计算最终跳跃的距离和容错（pb）
 * 4. calculateEndToStart：结束到开始的计算，用于Loop优化
 * 5. calculateBackToFrontUnit：向后转向前单位计算，用于判断是否可以用满助跑
 * 6. calculateRunJump：跑跳计算，计算起跳前跑1tick后的bm
 * 
 * 物理公式：
 * - 地面摩擦：v = v * 0.546 + 跳跃速度 + 地面移动速度
 * - 空中摩擦：v = v * 0.91 + 45度加速
 * - 45度加速：v += 0.018384775 * (sin + cos)
 * - 落地45度加速：v += 0.09192386 * (sin + cos)
 */
public class PhysicsCalculator {
     ...
     * 
     * 计算流程：
     * 1. 初始化：bm = 起始坐标 + 初始向后速度
     * 2. 起跳tick：
     *    - 如果fixPlan==4（特殊修复方案），使用fixSpeed
     *    - 否则：速度 = 速度 * 地面摩擦(0.546) + 跳跃速度(0.2) + 地面移动速度(0.1274)
     *    - bm += 速度
     * 3. 遍历每个连跳（除了最后一个）：
     *    - 如果不是第一个连跳，先进行普通跳跃tick（落地后起跳）
     *    - 第一个airtime使用45度加速（如果airtime >= 2）
     *    - 后续airtime ticks使用45度加速
     * 4. 最后处理：
     *    - 如果非delayed：减去最后一tick的速度（还没落地）
     *    - 如果delayed：落地时使用45度加速
     * 
     * @param initialBackwardSpeed 初始向后速度（s0）
     * @param delayed 是否使用delayed起跳（delayed起跳在落地时会有额外的45度加速）
     * @return 能够达到的向前bm（相对于起始坐标）
     */
     ...
     * 
     * 这个方法是优化过程中的核心，支持指定起跳速度，并处理移动阻断。
     * 
     * 与calculateJumpBM的区别：
     * 1. 可以指定起跳速度（jumpSpeed），而不是从初始速度计算
     * 2. 支持移动阻断检测和处理（Plan 1/2/3）
     * 3. 如果jumpSpeed是1或-1，返回bm；否则返回最终速度
     * 
     * 移动阻断处理：
     * - 检测地面阻断：起跳速度在±0.0091...范围内
     * - 检测空中阻断：空中速度在±0.0054...范围内
     * - 如果检测到阻断，根据fixPlan进行相应处理：
     *   - Plan 1：刚好慢于阻断，过阻断后全力前进
     *   - Plan 2：刚好卡在阻断下限，调整过阻断后的速度
     *   - Plan 3：刚好快于阻断，调整过阻断后的速度
     * 
     * @param initialSpeed 初始速度（s0）
     * @param jumpSpeed 起跳速度（jSpeed），如果为1或-1表示用于插值计算
     * @param finalDelayed 最后是否delayed起跳
     * @return 如果jSpeed是1或-1返回bm，否则返回最终速度（tempV0）
     */
     ...
     * 最终跳跃计算（对应原finaljump）
     * 
     * 计算从起跳速度v0开始，经过最终跳跃后的距离和容错（pb）。
     * 
     * 计算流程：
     * 1. 初始化距离：距离 = 起始坐标 + 玩家中心偏移 + 起跳速度
     * 2. 起跳tick：
     *    - delayed：使用地面摩擦
     *    - 非delayed：使用空中摩擦
     *    - 距离 += 速度
     * 3. 第一个airtime使用45度加速（如果airtime > 2）
     * 4. 后续airtime ticks使用45度加速
     * 5. 计算pb（容错）：
     *    - pb = 距离 - 距离向下取整到0.0625的倍数
     *    - pb表示距离与方块边界的差距，用于判断实际跳跃的容错
     * 
     * @param jumpVelocity 起跳速度（v0）
     * @param delayed 是否delayed起跳
     * @return 包含distance（最终距离）、pb（容错）、finalV0（最终速度）的结果
     */
